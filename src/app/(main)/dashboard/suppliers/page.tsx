...
function SupplierForm({
  ...
  defaultValues: SupplierDto | null;
  isLoading?: boolean;
}) {
  const form = useForm<FormValues>({
    resolver: zodResolver(schema) as Resolver<FormValues>,
    defaultValues: { name: "", contactName: "", email: "", phone: "", address: "", city: "", country: "" },
  });
  ...
  const fields: { name: keyof FormValues; label: string; placeholder?: string }[] = [
    ...
    { name: "email", label: "Email", placeholder: "contact@supplier.com" },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{defaultValues ? "Edit Supplier" : "New Supplier"}</DialogTitle>
        </DialogHeader>
        <form
    ...
    try {
      if (editTarget) {
        await updateSupplier(editTarget.id, {
          // biome-ignore lint/style/noNonNullAssertion: validated by zod schema
          name: values.name!,
