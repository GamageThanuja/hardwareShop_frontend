...
import { PurchaseReturnStatus, type PurchaseReturnDto, type PurchaseOrderDto, type WarehouseDto } from "@/lib/api/types";
...
function PurchaseReturnForm({ open, onOpenChange, onSubmit, isLoading, purchaseOrders, warehouses, products }: {
  open: boolean; onOpenChange: (o: boolean) => void; onSubmit: (v: FormValues) => Promise<void>;
  isLoading?: boolean; purchaseOrders: PurchaseOrderDto[]; warehouses: WarehouseDto[];
  products: { id: string; name: string | null; sku: string | null }[];
}) {
  const today = new Date().toISOString().split("T")[0];
  const form = useForm<FormValues>({
    resolver: zodResolver(schema) as Resolver<FormValues>,
    defaultValues: { purchaseOrderId: "", warehouseId: "", returnDate: today, reason: "", notes: "", items: [{ productId: "", quantity: 1, notes: "" }] },
  });
  const { fields, append, remove } = useFieldArray({ control: form.control, name: "items" });

  useEffect(() => {
    if (!open) form.reset({ purchaseOrderId: "", warehouseId: "", returnDate: today, reason: "", notes: "", items: [{ productId: "", quantity: 1, notes: "" }] });
  }, [open, form, today]);
        ...
        <form onSubmit={form.handleSubmit(async (v) => { await onSubmit(v); })} className="space-y-4">
          <FieldGroup className="gap-4">
            <div className="grid grid-cols-2 gap-4">
              <Controller control={form.control} name="purchaseOrderId" render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel>Purchase Order *</FieldLabel>
                  <Select value={field.value} onValueChange={field.onChange}>
                    <SelectTrigger><SelectValue placeholder="Select PO" /></SelectTrigger>
                    <SelectContent>
                      {purchaseOrders.map((po) => <SelectItem key={po.id} value={po.id}>{po.poNumber ?? po.id.slice(0, 8)}</SelectItem>)}
                    </SelectContent>
                  </Select>
...
export default function PurchaseReturnsPage() {
  ...
  useEffect(() => {
    listPurchaseOrders({ PageSize: 500 }).then((r) => setPurchaseOrders(r.data.items ?? [])).catch(() => {});
  ...
  const columns: ColumnDef<PurchaseReturnDto, unknown>[] = [
    ...
    { accessorKey: "poNumber", header: "PO #", cell: ({ getValue }) => <span className="font-mono text-xs">{(getValue() as string | null) ?? "—"}</span> },
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      toast.success("Purchase return cancelled."); setCancelTarget(null); load();
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader title="Purchase Returns" description="Manage returns to suppliers" action={
      ...
      <PurchaseReturnForm open={formOpen} onOpenChange={setFormOpen} onSubmit={handleSubmit} isLoading={isSaving} purchaseOrders={purchaseOrders} warehouses={warehouses} products={products} />
