...
import { PurchaseOrderStatus, type PurchaseOrderDto, type SupplierDto } from "@/lib/api/types";
...
function PurchaseOrderForm({ open, onOpenChange, onSubmit, isLoading, suppliers, products }: {
  open: boolean; onOpenChange: (o: boolean) => void; onSubmit: (v: FormValues) => Promise<void>;
  isLoading?: boolean; suppliers: SupplierDto[]; products: { id: string; name: string | null; sku: string | null }[];
}) {
  const today = new Date().toISOString().split("T")[0];
  const form = useForm<FormValues>({
    resolver: zodResolver(schema) as Resolver<FormValues>,
    defaultValues: { supplierId: "", orderDate: today, expectedDeliveryDate: "", notes: "", items: [{ productId: "", quantity: 1, unitCost: 0 }] },
  });
  const { fields, append, remove } = useFieldArray({ control: form.control, name: "items" });

  useEffect(() => {
    if (!open) form.reset({ supplierId: "", orderDate: today, expectedDeliveryDate: "", notes: "", items: [{ productId: "", quantity: 1, unitCost: 0 }] });
  }, [open, form, today]);
        ...
        <form onSubmit={form.handleSubmit(async (v) => { await onSubmit(v); })} className="space-y-4">
          <FieldGroup className="gap-4">
            <div className="grid grid-cols-2 gap-4">
              <Controller control={form.control} name="supplierId" render={({ field, fieldState }) => (
            ...
            <div className="grid grid-cols-2 gap-4">
              <Controller control={form.control} name="expectedDeliveryDate" render={({ field }) => (
                <Field><FieldLabel>Expected Delivery</FieldLabel><Input {...field} type="date" /></Field>
              )} />
            ...
            <div className="space-y-2">
              ...
              {fields.map((f, i) => (
                <div key={f.id} className="grid grid-cols-[1fr_80px_100px_32px] gap-2 items-end">
                  <Controller control={form.control} name={`items.${i}.productId`} render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                  ...
                  <Controller control={form.control} name={`items.${i}.unitCost`} render={({ field }) => (
                    <Field>
                      {i === 0 && <FieldLabel className="text-xs text-muted-foreground">Unit Cost</FieldLabel>}
...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    listSuppliers({ PageSize: 200 }).then((r) => setSuppliers(r.data.items ?? [])).catch(() => {});
  ...
  const columns: ColumnDef<PurchaseOrderDto, unknown>[] = [
    { accessorKey: "poNumber", header: "PO #", cell: ({ getValue, row }) => (
      <Link href={`/dashboard/purchase-orders/${row.original.id}`} className="font-mono text-xs hover:underline text-primary">
    ...
    { accessorKey: "orderDate", header: "Order Date", cell: ({ getValue }) => new Date(getValue() as string).toLocaleDateString() },
    { accessorKey: "expectedDeliveryDate", header: "Expected Delivery", cell: ({ getValue }) => (getValue() as string | null) ? new Date(getValue() as string).toLocaleDateString() : "—" },
    ...
    { id: "actions", header: "", cell: ({ row }) => (
      <div className="flex gap-2 justify-end">
        <Button variant="ghost" size="sm" asChild><Link href={`/dashboard/purchase-orders/${row.original.id}`}>View</Link></Button>
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Purchase order deleted."); setDeleteTarget(null); load();
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader title="Purchase Orders" description="Manage purchase orders from suppliers" action={
        <Button onClick={() => setFormOpen(true)}><Plus className="h-4 w-4 mr-2" />New Purchase Order</Button>
      } />
      <div className="flex gap-2 mb-4">
        <Input placeholder="Search purchase orders…" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="max-w-sm" />
      </div>
      ...
      <PurchaseOrderForm open={formOpen} onOpenChange={setFormOpen} onSubmit={handleSubmit} isLoading={isSaving} suppliers={suppliers} products={products} />
      <ConfirmDialog open={!!deleteTarget} onOpenChange={(o) => { if (!o) setDeleteTarget(null); }} title="Delete Purchase Order" description={`Delete PO "${deleteTarget?.poNumber}"?`} onConfirm={handleDelete} isLoading={isDeleting} confirmLabel="Delete" />
    </div>
  );
}
