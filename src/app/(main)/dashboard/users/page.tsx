...
function CreateUserForm({ open, onOpenChange, onSubmit, isLoading }: {
  open: boolean; onOpenChange: (o: boolean) => void; onSubmit: (v: CreateFormValues) => Promise<void>; isLoading?: boolean;
}) {
  ...
  useEffect(() => {
    if (!open) form.reset({ firstName: "", lastName: "", userName: "", email: "", phoneNumber: "", password: "", roles: [] });
  }, [open, form]);
      ...
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={form.handleSubmit(async (v) => { await onSubmit(v); })} className="space-y-4">
          <FieldGroup className="gap-3">
            ...
            <Controller control={form.control} name="roles" render={({ field }) => (
              <Field>
...
function EditUserForm({ open, onOpenChange, onSubmit, defaultValues, isLoading }: {
  open: boolean; onOpenChange: (o: boolean) => void; onSubmit: (v: EditFormValues) => Promise<void>;
  defaultValues: UserDto | null; isLoading?: boolean;
}) {
  ...
  useEffect(() => {
    if (defaultValues) {
      form.reset({ firstName: defaultValues.firstName ?? "", lastName: defaultValues.lastName ?? "", phoneNumber: defaultValues.phoneNumber ?? "", isActive: defaultValues.isActive, roles: defaultValues.roles ?? [] });
    }
  }, [defaultValues, form]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Edit User — {defaultValues?.userName}</DialogTitle></DialogHeader>
        <form onSubmit={form.handleSubmit(async (v) => { await onSubmit(v); })} className="space-y-4">
          <FieldGroup className="gap-3">
            <div className="grid grid-cols-2 gap-3">
              <Controller control={form.control} name="firstName" render={({ field }) => (
...
export default function UsersPage() {
  ...
  const columns: ColumnDef<UserDto, unknown>[] = [
    { accessorKey: "userName", header: "Username", cell: ({ getValue }) => <span className="font-mono text-sm">{getValue() as string}</span> },
    { id: "name", header: "Name", cell: ({ row }) => `${row.original.firstName ?? ""} ${row.original.lastName ?? ""}`.trim() || "—" },
    { accessorKey: "email", header: "Email" },
    { accessorKey: "roles", header: "Roles", cell: ({ getValue }) => {
      const roles = (getValue() as string[] | null) ?? [];
      return roles.length > 0 ? <div className="flex gap-1 flex-wrap">{roles.map((r) => <Badge key={r} variant="outline">{r}</Badge>)}</div> : "—";
    }},
    { accessorKey: "isActive", header: "Status", cell: ({ getValue }) => (
      <Badge variant={(getValue() as boolean) ? "default" : "secondary"}>{(getValue() as boolean) ? "Active" : "Inactive"}</Badge>
    )},
    { id: "actions", header: "", cell: ({ row }) => (
      <div className="flex gap-2 justify-end">
        <Button variant="ghost" size="sm" onClick={() => setEditTarget(row.original)}>Edit</Button>
  ...
  const handleCreate = async (values: CreateFormValues) => {
    setIsSaving(true);
    try {
      const user = await createUser({ firstName: values.firstName, lastName: values.lastName, userName: values.userName, email: values.email, phoneNumber: values.phoneNumber, password: values.password, roles: values.roles });
  ...
  const handleEdit = async (values: EditFormValues) => {
    ...
    setIsSaving(true);
    try {
      await updateUser(editTarget.id, { firstName: values.firstName, lastName: values.lastName, phoneNumber: values.phoneNumber, isActive: values.isActive });
  ...
  return (
    <div>
      <PageHeader title="Users" description="Manage system users and roles" action={
        <Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4 mr-2" />New User</Button>
      } />
      ...
      <EditUserForm open={!!editTarget} onOpenChange={(o) => { if (!o) setEditTarget(null); }} onSubmit={handleEdit} defaultValues={editTarget} isLoading={isSaving} />
      <ConfirmDialog open={!!deleteTarget} onOpenChange={(o) => { if (!o) setDeleteTarget(null); }} title="Delete User" description={`Delete user "${deleteTarget?.userName}"?`} onConfirm={handleDelete} isLoading={isDeleting} confirmLabel="Delete" />
    </div>
  );
}
